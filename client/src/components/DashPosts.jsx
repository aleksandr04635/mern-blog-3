...
export default function DashPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const res = await fetch(`/api/post/getpostsdash`);
  ...
  const handleShowMore = async () => {
    const startIndex = userPosts.length; //it's correct, every evocation of this function increases userPosts.length by 9
    try {
      const res = await fetch(
        //`/api/post/getpostsdash?userId=${currentUser._id}&startIndex=${startIndex}`,
        `/api/post/getpostsdash?startIndex=${startIndex}`,
      );
      const data = await res.json();
      if (res.ok) {
